...
# 11.2.7 Покупки в интернет-магазине 🌶️
'''
...
t = ['Вячеслав Ручка 1', 'Филипп Ручка 1', 'Виктория Перо 3', 'Вячеслав Линейка 4', 'Виктория Тетрадь 7', 'Вячеслав Ручка 29', 'Филипп Циркуль 1']

a = ['Виктория:', 'Перо 3', 'Тетрадь 7', 'Вячеслав:', 'Линейка 4', 'Ручка 30', 'Филипп:', 'Ручка 1', 'Циркуль 1']
...
d = dict()
for elem in t:
    name, item, c = elem.split()
    c = int(c)
    print(c)
    if name not in d:
        d.setdefault(name, {item : c})
    elif item not in d[name]:
        d[name].update({item:c})
    else:
        d[name][item] = d[name].get(item) + c
...
for elem in t:
    ...
    d[name][item] = d[name].get(item, 0) + int(c)

print(d)

for name, item in sorted(d.items()):
    print('{0}:'.format(name))
    for it, c in sorted(item.items()):
        print('{0} {1}'.format(it, c))
'''